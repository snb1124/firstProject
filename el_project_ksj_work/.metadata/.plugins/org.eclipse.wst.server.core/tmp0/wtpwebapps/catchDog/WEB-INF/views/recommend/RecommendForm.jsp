	...
	$(document).ready(function(){	
		...
		$("img[name=imgAupkind]").click(function(){ // 여러 객체 이벤트 동시
			...
			if(idNo == "01"){ // 고유값에 따른 처리 분기
				$("#imgAupkind_02").css("border", "10px solid #e8e8e8");
				$("#catsize").hide();
				$(".long").hide();
				$(".short").hide();
				if($("#dogsize").is(":visible")){
					$("#dogsize").slideUp("slow");					
					$("#imgAupkind_01").css("border", "10px solid #e8e8e8");
					$("#aupkind").val("");
					$("[name=asize]").attr("checked", false);
				} else {
					$("#dogsize").slideDown("slow");
					$("#imgAupkind_01").css("border", "10px solid #F7AA58");
					$("#aupkind").val(idNo);
				}
				...
				if($("#catsize").is(":visible")){
					...
					$("#imgAupkind_02").css("border", "10px solid #F7AA58");
					$("#aupkind").val(idNo);
				}
				$("[name=asize]").attr("checked", false);
				$("[name=akind]").attr("checked", false);
...
<jsp:include page="/include/header.jsp" flush="true"></jsp:include>
	...
	<form name="recommendForm" id="recommendForm" style="width: 700px; margin-left:auto; margin-right: auto;">
		<input type="hidden" name="mnum" id="mnum" value="<%= request.getAttribute("mnum") %>"/>
		<input type="hidden" name="aupkind" id="aupkind" value="" />
		<h1 style="font-size: 20px; text-align: center; color: black;">관심있는 반려동물을 선택해주세요</h1>
		<div style="margin-top: 20px;">
				<img name="imgAupkind" id="imgAupkind_01" src="image/recommendimg/dog.png" height="280" width="300" style="cursor: pointer;"/>
				<img name="imgAupkind" id="imgAupkind_02" src="image/recommendimg/cat.png" height="280" width="300"  style="cursor: pointer;"/>

		</div>
		<div id="dogsize" style="display:none;">
			<div class="asize" style="margin-top: 15px; text-align: center;">
				<label>
			...
			<div class="akind" style="margin-top: 15px; text-align: center;">
			<%
		...
		<div id="catsize" style="display:none;">
					<div class="afur" style="margin-top: 15px; text-align: center;">
						<label>
		...
		<div>
			<table style="width: 100%; margin: 20px 0; border-top: 1px solid #cccccc;">
				<tr>
					<td>성별</td>
					<td>
