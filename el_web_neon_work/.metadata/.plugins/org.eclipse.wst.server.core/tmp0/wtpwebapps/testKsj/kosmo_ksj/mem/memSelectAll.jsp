	...
	$(document).ready(function(){	
		...
		function send(isudType){
			...
			$("#memSelectAllForm").attr({
				"action":"/testKsj/mem?ISUD_TYPE="+isudType,
				"method":"GET",
...
<%
	...
	int nCnt = aList.size();
	out.println(" 전체 조회 건수는  " + nCnt + " 건 입니다.");	
%>
<table>
<tr>
<td calspan="2">
...
<%
	...
	for(int i=0; i<nCnt; i++){		
		...
		//F 나 M으로 넘어갈것 
		gender = CodeUtil.gender(mvo.getKgender());
		birth = CodeUtil.birth(mvo.getKbirth());
		hp = CodeUtil.hp(mvo.getKhp());
		addr = mvo.getKzonecode() + " " + mvo.getKroadaddress().replace("@", " " )+mvo.getKgibunaddress();
		hobby = CodeUtil.hobbys(mvo.getKhobby());				
%>					
<tbody>
<tr>
	<td class="tt">
		<input type="checkbox" id="knumCheck" name="knumCheck" value=<%= mvo.getKnum() %> onclick="checkOnly(this)">
	</td>		
	<td class="tt"><%= i + 1 %></td>
	<td class="tt"><%= mvo.getKnum() %> </td>
	<td class="tt"><%= mvo.getKname() %> </td>
	<td class="tt"><%= mvo.getKid() %> </td>
	<td class="tt"><%= gender %> </td>	
	<td class="tt"><%= birth %> </td>
	<td class="tt"><%= hp %> </td>
	<td class="tt"><%= mvo.getKemail() %> </td>
	<td class="tt"><%= addr %></td>
	<td class="tt"><%= hobby %></td>
	<!--  <td class="tt"><img src="/testKosmo/fileupload/member/<%= mvo.getKphoto() %> " border="1" width="25" height="25" alt="image"></td>-->	
